...
describe('Contradiction detection and supersedes edges (real)', () => {
  ...
  test('new contradicting decision creates supersedes edge on old one', async () => {
    ...
    if (supersededEdges.length > 0) {
      console.log(`[PASS] Contradiction detected! New decision supersedes ${supersededEdges.length} old decision(s)`)
      ...
      if (oldDecisions.length > 0) {
        console.log(`  Old decision marked superseded: "${oldDecisions[0].content.slice(0, 80)}..."`)
      }
    } else {
...
describe('buildContext with audience filtering (real)', () => {
  ...
  test('buildContext filters by audience and returns structured sections', async () => {
    ...
    console.log(`  Tokens: ${techContext.totalTokens}, Memories: ${techContext.memories.length}`)
    console.log(`  Context:\n${techContext.context}`)
    ...
    console.log('[PASS] Technical context: contains Kafka, excludes business-only cancellation policy')
    ...
    console.log(`[BUILD CONTEXT - ALL] Tokens: ${allContext.totalTokens}, Memories: ${allContext.memories.length}`)
...
describe('Direct write, entity graph, ask() and buildContext evaluation (real)', () => {
  ...
  test('populate knowledge graph and evaluate ask() responses', async () => {
    ...
    const orderProcessorId = await graph.createNode('module', {
      name: 'order-processor', path: 'services/order-processor', kind: 'service', status: 'active',
    })
    const paymentServiceId = await graph.createNode('module', {
      name: 'payment-service', path: 'services/payment-service', kind: 'service', status: 'active',
    })
    const orderEntityId = await graph.createNode('data_entity', {
      name: 'Order', source: 'services/order-processor/models/order.ts',
    })
    ...
    await graph.relate(orderProcessorId, 'connects_to', paymentServiceId, {
      protocol: 'sqs', direction: 'async', description: 'Sends payment requests after order validation',
    })
    ...
    console.log(`  [ASSESSMENT] Mentions SQS: ${mentionsSQS}, Mentions async/queue: ${mentionsAsync}`)
    ...
    const mentionsIsolation = answer2Lower.includes('isolation') || answer2Lower.includes('separate')
    console.log(`  [ASSESSMENT] Mentions PCI: ${mentionsPCI}, Compliance: ${mentionsCompliance}, Isolation: ${mentionsIsolation}`)
