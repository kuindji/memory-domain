...
import type { DomainConfig, DomainContext, OwnedMemory, RequestContext, SearchQuery } from '../src/core/types.ts'
...
describe('Topic + User domain coexistence (real)', () => {
  ...
  test('ingest user facts and topic, link them, verify graph structure', async () => {
    ...
    const fact1Id = await userCtx.writeMemory({
      content: 'The user has 5 years of experience with TypeScript and uses it daily for backend development',
      tags: [`${USER_TAG}/expertise`],
      ownership: { domain: USER_DOMAIN_ID, attributes: {} },
    })
    await userCtx.graph.relate(fact1Id, 'about_user', 'user:test-user', { domain: USER_DOMAIN_ID })
    await userCtx.graph.relate(fact1Id, 'about_topic', topicId, { domain: USER_DOMAIN_ID })

    const fact2Id = await userCtx.writeMemory({
      content: 'The user prefers strict TypeScript configuration with no-any and strict null checks enabled',
      tags: [`${USER_TAG}/preference`],
      ownership: { domain: USER_DOMAIN_ID, attributes: {} },
    })
    await userCtx.graph.relate(fact2Id, 'about_user', 'user:test-user', { domain: USER_DOMAIN_ID })
    ...
    console.log('[PASS] Graph structure: user node has 2 incoming edges, both facts link to topic')
  })
  ...
  test('vector search finds semantically similar content across domains', async () => {
    ...
    for (const entry of results.entries) {
      console.log(`  score=${entry.score.toFixed(3)} content="${entry.content.slice(0, 80)}..."`)
    }

    // At least one result should be about TypeScript
    const hasRelevant = contents.some(c =>
      c.toLowerCase().includes('typescript') || c.toLowerCase().includes('typed')
    )
...
describe('Visibility with built-in domains (real)', () => {
  ...
  beforeAll(async () => {
    ...
    await engine.ingest('User domain: knowledge about functional programming paradigms', { domains: [USER_DOMAIN_ID] })
    await engine.ingest('Topic domain: Rust language features and memory safety', { domains: [TOPIC_DOMAIN_ID] })
    const notesResult = await engine.ingest('Notes domain: meeting notes from yesterday about project timeline', { domains: ['notes'] })
  ...
  test('user domain sees only itself and topic, not notes', async () => {
    ...
    const results = await ctx.search({ text: 'meeting notes project timeline', mode: 'vector' })
    const contents = results.entries.map(e => e.content)
  ...
  test('getNodeEdges filters edges from non-visible domains', async () => {
    ...
    await userCtx.graph.relate(notesMemoryId, 'about_user', 'user:vis-user', { domain: 'notes' })
    ...
    await userCtx.graph.relate(userFactId, 'about_user', 'user:vis-user', { domain: USER_DOMAIN_ID })
    ...
    expect(sourceIds.some(id => id === userFactId || id.includes(userFactId.replace('memory:', '')))).toBe(true)
    expect(sourceIds.some(id => id === notesMemoryId || id.includes(notesMemoryId.replace('memory:', '')))).toBe(false)
  })
})
...
describe('End-to-end lifecycle (real)', () => {
  ...
  test('full lifecycle: ingest, link, search, consolidate, merge', async () => {
    ...
    const tsTopicId = await topicCtx.writeMemory({
      content: 'TypeScript is a statically typed programming language that builds on JavaScript with type safety',
      tags: [TOPIC_TAG],
      ownership: {
        domain: TOPIC_DOMAIN_ID,
        attributes: {
    ...
    const rustTopicId = await topicCtx.writeMemory({
      content: 'Rust is a systems programming language focused on memory safety, concurrency, and performance without garbage collection',
      tags: [TOPIC_TAG],
      ownership: {
        domain: TOPIC_DOMAIN_ID,
        attributes: {
    ...
    const tsFact = await userCtx.writeMemory({
      content: 'The user has been writing TypeScript professionally for 4 years in Node.js backends',
      tags: ['fact'],
      ownership: { domain: USER_DOMAIN_ID, attributes: {} },
    })
    await userCtx.graph.relate(tsFact, 'about_user', 'user:e2e-user', { domain: USER_DOMAIN_ID })
    ...
    await userCtx.graph.relate(rustFact, 'about_user', 'user:e2e-user', { domain: USER_DOMAIN_ID })
    await userCtx.graph.relate(rustFact, 'about_topic', rustTopicId, { domain: USER_DOMAIN_ID })
    ...
    const summary = allUserMemories.find(m =>
      m.id !== tsFact && m.id !== rustFact
    )
    ...
    console.log('[E2E MERGE] Both TypeScript and Rust topics remain active (dissimilar content)')
  })
})
