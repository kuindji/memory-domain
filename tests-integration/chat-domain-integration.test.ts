...
import { CHAT_DOMAIN_ID, CHAT_MESSAGE_TAG, CHAT_EPISODIC_TAG, CHAT_SEMANTIC_TAG } from '../src/domains/chat/types.ts'
...
describe('Chat message ingestion with cross-domain interaction (real)', () => {
  ...
  test('ingest chat messages, verify tags, attributes, and topic linking', async () => {
    ...
    await userCtx.graph.relate(factId, 'about_user', 'user:test-user', { domain: USER_DOMAIN_ID })
...
describe('Working memory promotion (real)', () => {
  ...
  test('promote working memory to episodic when capacity exceeded', async () => {
    ...
    console.log(`[PASS] Promotion complete: ${remainingWorking.length} working memories remain (capacity=2)`)
  })
})
...
describe('Full chat lifecycle (real)', () => {
  ...
  test('full lifecycle from ingestion through consolidation and pruning', async () => {
    ...
    console.log(`[LIFECYCLE] Phase 2: Promotion created ${episodicAfterPromote.length} episodic memories`)
    ...
    console.log(`[LIFECYCLE] Phase 3: Consolidation created ${semanticMemories.length} semantic memories`)
    ...
    console.log(`[LIFECYCLE] Phase 4: After pruning, ${episodicAfterPrune.length} episodic memories remain`)
    ...
    console.log(`[LIFECYCLE] Domain isolation: chat=${chatSearch.entries.length}, user=${userSearch.entries.length}, topic=${topicSearch.entries.length}`)
  ...
  test('buildContext returns tiered sections and isolates users', async () => {
    ...
    await chatCtx.writeMemory({
      ...
      ownership: {
        ...
        attributes: { role: 'user', layer: 'working', userId: 'alice', chatSessionId: 'alice-session', messageIndex: 0 },
      },
    })
    await chatCtx.writeMemory({
      ...
      ownership: {
        domain: CHAT_DOMAIN_ID,
        attributes: { role: 'user', layer: 'working', userId: 'alice', chatSessionId: 'alice-session', messageIndex: 1 },
      },
    })

    // --- Populate working memories for Bob (current session) ---
    await chatCtx.writeMemory({
      content: 'Bob: I am setting up database replication for high availability with PostgreSQL',
      tags: [CHAT_MESSAGE_TAG],
      ownership: {
        domain: CHAT_DOMAIN_ID,
        attributes: { role: 'user', layer: 'working', userId: 'bob', chatSessionId: 'bob-session', messageIndex: 0 },
      },
    })
    await chatCtx.writeMemory({
      ...
      ownership: {
        domain: CHAT_DOMAIN_ID,
        attributes: { role: 'user', layer: 'working', userId: 'bob', chatSessionId: 'bob-session', messageIndex: 1 },
      },
    })
    ...
    await chatCtx.writeMemory({
      content: 'Alice is a frontend developer who works extensively with React, Next.js, and Tailwind CSS',
      tags: [CHAT_SEMANTIC_TAG],
      ownership: {
        domain: CHAT_DOMAIN_ID,
