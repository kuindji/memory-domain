...
const entitiesDomain: DomainConfig = {
    ...
    schema: {
        ...
        edges: [{ name: "belongs_to", from: ["entity"], to: "category" }],
    },
...
describe("Multi-domain integration", () => {
        ...
        schema: {
            ...
            edges: [
                ...
                },
                {
                    ...
                    from: "memory",
                    to: "entity",
                    fields: [{ name: "role_in_context", type: "string" }],
                },
            ],
        },
        async processInboxBatch(entries: OwnedMemory[], ctx: DomainContext) {
            for (const entry of entries) {
                if (entry.memory.content.includes("alpha")) {
                    const topicId = await ctx.graph
                        .createNodeWithId("topic:alpha_topic", {
                            name: "Alpha Topic",
                            status: "active",
                        })
            ...
            edges: [
                {
                    ...
                    from: "memory",
                    to: "resource",
                    fields: [{ name: "direction", type: "string" }],
                },
            ],
        },
        async processInboxBatch(entries: OwnedMemory[], ctx: DomainContext) {
            for (const entry of entries) {
                if (entry.memory.content.includes("beta")) {
                    const resourceId = await ctx.graph
                        .createNodeWithId("resource:beta_resource", {
                            name: "Beta Resource",
                            kind: "abstract",
                        })
                        .catch(() => "resource:beta_resource");
                    await ctx.graph.relate(entry.memory.id, "impacts", resourceId, {
                        direction: "positive",
                    });
                }
            }
    ...
    test("entities domain allows creating shared node types", async () => {
        ...
        const categoryId = await graph.createNodeWithId("category:testing", {
            name: "Testing",
            kind: "group",
        });
