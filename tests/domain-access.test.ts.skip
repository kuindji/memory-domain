...
describe("Domain access enforcement", () => {
    ...
    test("writeMemory rejects for read-only domain", async () => {
        ...
        expect(engine.writeMemory("test", { domain: "ro" })).rejects.toThrow(
            'Domain "ro" is registered as read-only',
        );
    });
    ...
    test("ingest errors when all requested domains are read-only", async () => {
        ...
        expect(engine.ingest("test", { domains: ["ro"] })).rejects.toThrow("read-only");
    });
    ...
    test("relate rejects for read-only domain", async () => {
        ...
        expect(engine.relate(result.id!, "tag:test", "about_topic", "ro")).rejects.toThrow(
            'Domain "ro" is registered as read-only',
        );
    });
    ...
    test("releaseOwnership rejects for read-only domain", async () => {
        ...
        expect(engine.releaseOwnership("memory:fake", "ro")).rejects.toThrow(
            'Domain "ro" is registered as read-only',
        );
    });
