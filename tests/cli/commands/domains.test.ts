...
describe('domainCommand - skill subcommand', () => {
  ...
  it('returns error when skill is not found', async () => {
    const result = await domainCommand(engine, makeParsed('domain', ['test-domain', 'skill', 'nonexistent']))
    expect(result.exitCode).toBe(1)
    const output = result.output as { error: string }
...
describe('domainCommand - error cases', () => {
  ...
  it('returns error when domain does not exist', async () => {
    const result = await domainCommand(engine, makeParsed('domain', ['unknown-domain', 'structure']))
    expect(result.exitCode).toBe(1)
    const output = result.output as { error: string }
  ...
  it('returns error for unknown subcommand', async () => {
    const result = await domainCommand(engine, makeParsed('domain', ['test-domain', 'unknown']))
    expect(result.exitCode).toBe(1)
    const output = result.output as { error: string }
