...
| **Phase 7** LongMemEval retarget | **PARKED** | Adapter + rule-based scorer landed (commit `179b9fa`) but the dataset pass is parked with all LLM-adjacent work. Remains the scale-generalization check for 2.9 and unblocker for 2.11 retry once the park is lifted. |
| **Phase 2.13** Encoder upgrade round 2 | **DONE — Outcome A** | BGE-small → **BGE-base** (new default) + BGE-large (opt-in). Tier-2 eval-B coherence lifts 1/4 → **2/4** on both — matches the Phase-2.10 pass criterion that no anchor-scoring primitive ever hit. Eval-A lifts +0.022 (bge-base, Phase-2.8 default row) or +0.095 (bge-large, but BFS flips in as the winning traversal). Phase-2.8's "sessionDecay off-by-default" is encoder-stale — under bge-base/large, `decay=0.3` is load-bearing for the 2/4 coherence. See CONTEXT.md § Phase 2.13. |
| **Phase 2.14** Anchor-primitive retune under bge-base | **DONE — Outcome A'** | Narrow 1D sweep on `sessionDecayTau`, `weightedFusionTau`, `anchorTopK` under the Phase-2.13 bge-base default. Decay curve is non-monotonic: `decay=0.2` (or 0.1) lifts tier-2 eval-B coherence 2/4 → **3/4** (Athens-at-war arc flips 0.33 → 0.67); 0.3 is the 2/4 plateau; ≥0.4 collapses to 1/4. `wfusion τ` and `anchorTopK` variations are flat or regressive. Eval-A unchanged (sessionDecayTau inert on single-turn queries). New recommended session knob: `sessionDecayTau: 0.2`. Alexander-succession arc stays at 0.33 — new eval-B ceiling; next session's open question. See CONTEXT.md § Phase 2.14. |
| **Phase 2.14 Stage 2** Alexander-succession arc retest under bge-base | **DONE — Outcome N (refutation)** | Narrow retest of Option H (cluster-affinity-boost, 5 rows) and Option A1 (temporalDecayTau, 3 rows) scoped to the failing arc; PER_ARC diagnostic extended with missing/unexpected claim IDs + text. All 9 rows produce **byte-identical missing-claim signature** on Alexander-succession (cov=0.33 frozen): `diad_seleucus_babylon` + `diad_cassander_macedon` missing; `diad_wars_begin` + `diad_ptolemaic_dynasty` crowd in. A1 is encoder-invariant inert; H is net-harmful at β=1.0 (philosophers arc regresses 1.00 → 0.00). Arc is encoder-granularity-bound, not anchor-scoring-bound. Ships PER_ARC claim-level diagnostic + `ARC=<substring>` filter + `CONFIG_SET=phase214-stage2` matrix as reusable infrastructure. No default changes. Next = bge-large retune (Phase 2.13 Q2) or accept 3/4 ceiling. See CONTEXT.md § Phase 2.14 Stage 2. |
| **Phase 2.15** BGE-large retune | **DONE — Outcome N (refutation)** | Full 1D sweep under bge-large holds tier-2 eval-B coherence flat at **2/4** across 13 rows (decay / weightedFusionTau / anchorTopK / dijkstra-tmp). Node/edge bump counts are **byte-identical** across 8 decay+τ rows (14153/14088) — anchor-layer knobs are post-encoder-noise under bge-large's 1024d geometry. Eval-A holds 0.722 across all BFS rows (+0.095 over Phase-2.8 baseline); Dijkstra regresses across the full tmp band (0.573–0.626), confirming H4 — Phase 2.13's Dijkstra-regress is structural, not tmp-0.5-specific. Alexander-succession missing-claim signature differs from bge-base (`diad_ptolemy_egypt` + `diad_seleucus_babylon` missing; `diad_wars_begin` + `diad_lysimachus_thrace` crowd in) but cov still 0.33 — encoder-sensitive but not encoder-resolvable. Phase 2.13 Q2 answered: +0.095 eval-A is the ceiling for a bge-large phase; no anchor-scoring primitive opens within it. Ships `CONFIG_SET=phase215` matrix in-tree. No default changes. Next = Phase 2.16 multi-encoder ensemble or cross-encoder reranker. See CONTEXT.md § Phase 2.15. |
