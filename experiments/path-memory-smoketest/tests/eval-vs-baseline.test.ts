...
// Rank claim ids across paths: each claim's score = score of the highest-scoring path containing it.
...
describe("eval (A) — path retriever vs flat vector baseline (tier 1)", () => {
    test("path retriever beats baseline on tier-1 queries", async () => {
        ...
        for (const q of queriesTier1) {
            ...
            rows.push(
                `  [${outcome}] k=${k.toString().padStart(2)}  ${q.name.padEnd(34)}  path F1=${pathScore.f1.toFixed(2)}  base F1=${baselineScore.f1.toFixed(2)}`,
            );
        }

        const total = queriesTier1.length;
        console.log(`\n=== Tier 1 eval-vs-baseline (P/R @ K=|ideal|) ===`);
        console.log(
